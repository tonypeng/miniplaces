...
def resid_group(x, is_training, ksize, chan_out, num_resid, downsize_factor=2):
    resid1 = None
    chan_in = x.get_shape()[-1]
    if chan_in == chan_out:
        resid1 = resid_block(x, is_training, ksize)
    else:
        resid1 = resid_block_dim_increase(x, is_training, ksize, downsize_factor, chan_out)
...
def fully_connected(x, num_outputs, init_weights_stddev=1.0, init_bias_value=0):
    num_inputs = x.shape()[1]
    W = initialize_weights([num_inputs, num_outputs])
    b = initialize_biases([num_outputs], 0)
